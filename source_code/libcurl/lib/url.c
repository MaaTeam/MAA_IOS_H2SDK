                     ...
                     va_list param)
{
  ...
  case CURLOPT_PIPEWAIT:
    ...
    break;
  case CURLOPT_MAA_ENABLE:
      Curl_maa_on_enable(data, (0 != va_arg(param, long))?TRUE:FALSE);
    break;
  case CURLOPT_MAA_HTTPS_SWITCH:
      Curl_maa_on_https_switch(data, (0 != va_arg(param, long))?TRUE:FALSE);
    break;
  default:
  ...
  *waitpipe = FALSE;
  ...
  if(bundle) {
    ...
    while(curr) {
      ...
      if(!Curl_maa_check_connection_reuseable(needle, check))
        continue;
                         ...
                         bool *protocol_done)
{
  ...
  *protocol_done = FALSE; /* default to not done */
  ...
  if(CURL_SOCKET_BAD == conn->sock[FIRSTSOCKET]) {
    ...
    Curl_maa_check_connection_type(conn); /* Check CNAME and decide whether use MAA or not*/
